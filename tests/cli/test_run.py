...
def run_and_terminate_server(run_cli_command, capfd):
    ...
    def _run_and_terminate_server(**kwargs):
        """Run the server using `aiida-optimade run`
...
@pytest.mark.skipif(
    os.getenv("CI") == "true", reason="Base URL is different in GitHub Actions CI."
)
...
def test_log_level_warning(run_and_terminate_server):
    ...
    assert "DEBUG:" not in output.err
...
@pytest.mark.skipif(
    os.getenv("CI") == "true", reason="Issues with capfd in GitHub Actions CI."
)
def test_env_var_is_set(run_and_terminate_server, capfd):
    ...
    Hence, we test it by making sure the current AiiDA profile (the test profile
    `test_profile`) is reported to be the active profile when running the server.

    Since `run_and_terminate_server` automatically sets the "AIIDA_PROFILE"
    environment variable to "optimade_sqla", we will check that here.
    """

    fixture_profile = "optimade_sqla"
    options = ["--debug"]
    run_and_terminate_server(options=options)
    assert fixture_profile in capfd.readouterr().out
