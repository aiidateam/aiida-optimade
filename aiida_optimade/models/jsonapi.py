...
class ErrorLinks(BaseModel):
    ...
    about: Optional[Union[None, UrlStr, Link]] = Schema(
        ...,
...
class RelationshipLinks(BaseModel):
    ...
    def at_least_one_must_be_specified(cls, value, values):
        if values.get("self", None) is None and value is None:
            raise AssertionError(
                "Either 'self' or 'related' MUST be specified for ResourceLinks"
            )
        return value
...
class Relationship(BaseModel):
    ...
    def at_least_one_relationship_key_must_be_set(cls, value, values):
        if (
...
class Response(BaseModel):
    ...
    def either_data_meta_or_errors_must_be_set(cls, value, values):
        if (
            ...
            raise AssertionError(
                "Either 'data', 'meta', or 'errors' must be specified at the top-level"
            )
        return value
